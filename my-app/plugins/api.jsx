...
import { wrapper as store } from "../store/store"

const API_BASE_URL =
	(process && process.env && process.env.API_BASE_URL) ||
	"http://localhost:1337"
...
export default {
	...
	post(url, body, options = {}) {
		return new Promise((resolve, reject) => {
			...
			axios
				.post(API_BASE_URL + url, body, options)
				.then((response) =>
					handleResponse(resolve, reject, options, response)
				)
				.catch((error) => {
	...
	put(url, body, options = {}) {
		return new Promise((resolve, reject) => {
			...
			axios
				.put(API_BASE_URL + url, body, options)
				.then((response) =>
					handleResponse(resolve, reject, options, response)
				)
				.catch((error) => {
